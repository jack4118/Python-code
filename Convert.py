    ...
    if y >= 1:
        if (paramTest[y]-paramTest[y-1]) > 0:
            if arrayTest[y] == 0:
                ...
                arrayTest[y] = -1
            print('ParamValue=>',paramValue,' Executing=>',ExecutingParam)
...
for TestLoop in range(lenTest): 
    ...
    for i in range(temp):
        if temp >= 1:
            if i < 1:
                print('1TempC ->', TempCount, ' TargetC ->',StartCount)
    ...
    pTemp = 0
    # UsedCount = 0
    print('pVar ->',pVariable, ' p->',p,' total_te->', total_test)
    print('count->',count,' TargetCount->', TargetCount, ' EndTar->',EndTargetCount,' StartCount->',StartCount)
        ...
        for line in file_contents:
            if total_test == 0:
                if i < pArray:
                    if count >= StartCount:
                        if count<=EndTargetCount: 
                            if 'device name="' in line:
                                ...
                                print("1Array ",i,":",Par[i],' Value:', Val[i], ' line', count +1)
                                # print('x->', x, ' pArray->',pArray)
                    ...
                    print('Total_Test->',total_test,' i->',i,' x->',x)
                    total_test += 1
                    TargetCount = count
                    count = 0
                    i -= 1
                    print('Total_Test->',total_test,' i->',i,' x->',' Executing test->',TestExecuting)
                    print(i + EndTest[x])
                    break 
            elif total_test >= 1:
                # if i < (pArray[x] + pArray[x-1] - pTemp):
                if i < size:
                    if count >= TargetCount:
                        if count<=EndTargetCount:
                            if 'test name="' in line:
                            ...
                            #     p -= 1
                                ...
                                if 'Used' not in Par[TestExecuting]:
                                    print('CURRENT->',x)
                                    print("2Array ",i,":",Par[TestExecuting],' Value:', Val[TestExecuting], ' line', count +1)
